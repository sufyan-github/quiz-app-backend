...
exports.aiService = {
    async askTutor(prompt, userId, topicId, lessonId) {
        const template = await prisma_1.prisma.aiPromptTemplate.findUnique({
        ...
        await prisma_1.prisma.aiConversation.create({
            data: {
                userId,
    ...
    async generateHint(questionId, userId) {
    ...
    Options: ${question.options.map(o => o.text).join(', ')}`;
        ...
        await prisma_1.prisma.aiFeedback.create({
            data: {
                userId,
                targetType: 'QUESTION',
                targetId: questionId,
    ...
    async generateQuiz(topicId, adminId, difficulty, count, adminPrompt, language) {
    ...
    Each object must have exactly this structure:
    {
      ...
      "options": [
        ...
        const generated = await prisma_1.prisma.aiGeneratedQuestion.create({
            data: {
                topicId: topic.id,
                generatedBy: adminId,
