...
exports.deleteQuestion = exports.addQuestionToExam = exports.getAllResults = exports.generateCertificate = exports.submitExam = exports.saveAnswer = exports.startExam = exports.deleteExam = exports.updateExam = exports.createExam = exports.getExams = void 0;
...
const createExam = async (req, res) => {
    try {
        ...
        const questionIds = Array.isArray(req.body.questionIds)
            ? req.body.questionIds.filter((id) => typeof id === 'string')
            : [];
        ...
        const validQuestionCount = await prisma_1.default.question.count({
...
const deleteExam = async (req, res) => {
    try {
        const id = req.params.id;
        const exam = await prisma_1.default.exam.update({ where: { id }, data: { status: 'ARCHIVED' } });
...
const startExam = async (req, res) => {
    try {
        ...
        const attemptResult = await prisma_1.default.$transaction(async (tx) => {
            ...
            if (exam.maxAttempts != null && completedAttempts >= exam.maxAttempts)
...
const saveAnswer = async (req, res) => {
    try {
        ...
        if (selectedOptionId) {
            const option = await prisma_1.default.option.findFirst({ where: { id: selectedOptionId, questionId } });
            if (!option) {
                res.status(400).json({ error: 'Option does not belong to this question' });
                return;
            }
        }
        const answer = await prisma_1.default.studentAnswer.upsert({
    ...
    try {
        ...
        for (const item of attempt.exam.questions) {
            ...
            if (isCorrect) {
                ...
                if (answer)
                    answerScores.push({ id: answer.id, isCorrect: true, marksAwarded: question.marks });
            }
            else {
                wrongCount += 1;
                ...
                if (answer)
                    answerScores.push({ id: answer.id, isCorrect: false, marksAwarded: -question.negativeMarks });
            }
        }
...
const addQuestionToExam = async (req, res) => {
    try {
        ...
        if (typeof req.body.text !== 'string' || options.length < 2 || options.filter((option) => option?.isCorrect === true).length !== 1) {
            res.status(400).json({ error: 'Text, two options, and exactly one correct option are required' });
            return;
        }
        const last = await prisma_1.default.examQuestion.findFirst({ where: { examId }, orderBy: { order: 'desc' } });
        const question = await prisma_1.default.question.create({
            data: {
                ...
                options: { create: options.map((option) => ({ text: String(option.text || ''), isCorrect: option.isCorrect === true })) },
...
const deleteQuestion = async (req, res) => {
    try {
        ...
        const question = await prisma_1.default.question.update({ where: { id: questionId }, data: { status: 'ARCHIVED' } });
