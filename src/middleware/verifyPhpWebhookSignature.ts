...
export function verifyPhpWebhookSignature(req: Request, res: Response, next: NextFunction): void {
  ...
  const rawBody = (req as any).rawBody instanceof Buffer ? (req as any).rawBody.toString('utf8') : JSON.stringify(req.body || {});
  const payloadToSign = rawBody + timestamp + nonce;
  const expectedSignature = crypto.createHmac('sha256', INTERNAL_API_KEY).update(payloadToSign).digest('hex');

  const providedBuf = Buffer.from(signature);
  const expectedBuf = Buffer.from(expectedSignature);
  const validSignature = providedBuf.length === expectedBuf.length && crypto.timingSafeEqual(providedBuf, expectedBuf);
