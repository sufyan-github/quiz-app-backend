...
interface CacheEntry { questions: DemoQuestion[]; expiresAt: number; }
...
function cacheKey(category: string, subject: string, topic: string, language: string): string {
  return `${category}::${subject.toLowerCase()}::${topic.toLowerCase()}::${language}`;
}

function getCached(key: string): DemoQuestion[] | null {
...
function setCached(key: string, questions: DemoQuestion[]): void {
...
function isValidQuestionSet(data: any): data is DemoQuestion[] {
...
export const demoController = {
  async generateDemoQuiz(req: Request, res: Response): Promise<void> {
    try {
      ...
      const subject = sanitizeFreeText(String(req.body.subject || ''), 60);
      const topic = sanitizeFreeText(String(req.body.topic || ''), 80);
      if (!subject) {
