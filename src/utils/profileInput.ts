...
export function sanitizeProfileInput(input: unknown): Record<string, unknown> {
  ...
  for (const field of PROFILE_FIELDS) {
    ...
    if (field === 'dob' || field === 'joiningDate') {
      if (typeof value === 'string' && !Number.isNaN(Date.parse(value))) result[field] = new Date(value);
      else if (value === null) result[field] = null;
      continue;
    }
    if (typeof value === 'string') result[field] = value.trim().slice(0, 2000);
    else if (value === null) result[field] = null;
  }
