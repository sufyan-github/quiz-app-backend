...
export const demoTaxonomy: Record<string, DemoCategory> = {
  cse: {
...
export function sanitizeFreeText(input: string, maxLen: number): string {
  const cleaned = (input || '')
...
export function resolveCategory(categoryKey: string): DemoCategory | null {
  return demoTaxonomy[categoryKey] || null;
}
